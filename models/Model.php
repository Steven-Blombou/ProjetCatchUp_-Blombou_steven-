...
abstract class Model
{
  ...
  private static function setBdd(){
    self::$_bdd = new PDO('mysql:host=localhost;dbname=catch_up;charset=utf8', 'root', '');
  ...
  protected function getOne($table, $obj, $id)
  {
    $this->getBdd();
    $var = [];
    $req = self::$_bdd->prepare("SELECT id_article, autor, title, content, DATE_FORMAT(date, '%d/%m/%Y à %Hh%imin%ss') AS date FROM " .$table. " WHERE id = ?");
